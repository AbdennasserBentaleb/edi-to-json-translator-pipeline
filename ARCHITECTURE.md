...
The EDI to JSON Translator Pipeline is built using Java 17, Spring Boot, and Apache Camel. It implements Enterprise Integration Patterns (EIP) to provide robust, idempotent translation of legacy Electronic Data Interchange (EDI) files into modern canonical JSON payloads.
...
## Deployment
The pipeline is designed as a stateless 12-Factor application. Secrets, ports, and directory mappings are passed as environment variables.
Deployment manifests for Kubernetes (`Deployment` and `Service`) are provided in the `k8s/` directory.

-   **Containerization**: A `Dockerfile` is provided using a lightweight JDK image.
-   **Volumes**: Kubernetes PersistentVolumes (PV) or emptyDir can be mounted to `/app/inbox` and `/app/error` for file persistence.
